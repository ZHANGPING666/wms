...
function addTab(contentFather, tabs, iframeid, name, url) {
    ...
    //如果没有这个页签则添加页签
    var tab = $('<div class="btn btn-success tab"  iframeid="' + iframeid + '">' + name + '&nbsp;&nbsp;<span class="glyphicon glyphicon-remove"></span></div>$nbsp;');
    tab.appendTo(tabs);
    tab.siblings().attr('class', 'btn btn-info tab');//改变其他标签颜色
    //添加内容
...
function showTab(contentFather, tabs, iframeid) {
    //改变当前页签颜色
    var tab=$(tabs.find('[iframeid=' + iframeid + ']')).attr('class', 'btn btn-success tab');
    //改变其他页签颜色
    tab.siblings().attr('class', 'btn btn-info tab');
    //把对应页置于最上层
